...
ip = input("输入这个节点的 IP: ")
blockchain = Blockchain(utils.get_magic_param()[0], utils.get_magic_param()[1], ip)  # 实例化类
...
def get_map():
    if blockchain.ip == ROOT:
        ...
        if blockchain.ip in r:
            r.remove(blockchain.ip)
        else:
            requests.post(f'http://{ROOT}/register_nodes', {'node': blockchain.ip})
...
def transaction():
    ...
    if values['recipient'] == '' or values['amount'] == '':
        ...
        if blockchain.sub_transactions(values['recipient'], int(values['amount'])):
